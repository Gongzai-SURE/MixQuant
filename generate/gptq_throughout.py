...
def generate_hf(model: BaseGPTQForCausalLM, input_ids, n_generate):
...
def run_round(generator, model_path, quant_file, n_generate, input_ids, batch_size, no_safetensors, pretrained):
    ...
    model = AutoGPTQForCausalLM.from_quantized(model_path, device="cuda:0", use_triton=False)
    model = model.model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="/root/autodl-tmp/models/llama2-7b-gptq", help="path to the model")
    ...
    parser.add_argument("--pretrained", default=True, action="store_true", help="Measure pretrained model.")
